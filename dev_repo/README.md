...
从运行层升级开始，`dev_repo/` 不只保存过程真相，也保存架构真相。
...
架构图的合适详细度不是“每个函数都画进去”，而是 **agent 可行动架构**：
...
接手旧项目时，如果缺少 `dev_repo/architecture/`，应先做架构普查。普查可以承认不确定性：

- `confirmed`：由代码、配置、脚本或运行证据直接证明
- `inferred`：由命名、目录、调用或提交历史推断
- `unknown`：需要后续探针或人类确认

架构可以变，但不能偷变。凡是改变边界、职责、公共接口、依赖方向、运行流、持久化/证据模型或宿主适配语义的改动，都应进入架构修宪合同。
