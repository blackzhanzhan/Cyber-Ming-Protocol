...
- [`第一次上手指南`](wiki/00-开始这里与落地形态/复制即用提示词包.md)：第一次就走这里；最小路由、闭环图、推荐 / 万能 / 纠偏 / Skill 试用都在同一页
...
第一次上手先看 [`第一次上手指南`](wiki/00-开始这里与落地形态/复制即用提示词包.md)；想看详细方法论展开，再看 [`最小闭环`](wiki/02-最小闭环与核心礼法/最小闭环：一次审计版与多次审计版.md)。看图时只记住一句：Web 不直接放行施工，Web 只回判断；真正放行与成立完成事实，都在人类手里。
...
**00-入口：**
- [第一次上手指南](wiki/00-开始这里与落地形态/复制即用提示词包.md)：最小路由、最小闭环、推荐版、万能版、纠偏词、Skill 试用都在这里
...
- [原子执行合同与起居注](wiki/02-最小闭环与核心礼法/核心礼法之一：原子级任务清单与赛博起居注.md)：为什么它比 spec 更轻、更硬、更可审计
...
- [First-Time Guide](wiki-en/00-entry/prompt-pack.md): start here for your first run; the minimal route, loop diagram, recommended mode, universal mode, correction prompts, and Skill trial all live on one page
...
Start with the [First-Time Guide](wiki-en/00-entry/prompt-pack.md) for your first run; use [Minimal Loop](wiki-en/02-how/minimal-loop.md) when you want the expanded methodology. Read the diagram with one rule in mind: the Web side does not directly release implementation; it only returns judgment, and the human remains the sovereign router and final arbiter.
...
- [Atomic Execution Contract and Chronicles](wiki-en/02-how/atomic-checklist-chronicles.md): why the contract form is lighter than heavy specs and stronger than ordinary plans
