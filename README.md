...
- [`第一次上手指南`](wiki/00-开始这里与落地形态/复制即用提示词包.md)：先用两个提示词跑通一轮最小闭环，先证明这套协议能跑
- [`最小稳定闭环指南`](wiki/00-开始这里与落地形态/最小稳定闭环指南.md)：跑通后走这里；IDE 端 Skill 与 Web 端固定提示词 / 专有应用必须同时就位，release 也要双端成对同步
- [`推荐版直达`](wiki/00-开始这里与落地形态/复制即用提示词包.md#repo-link-mode)：能读 repo 链接、`webfetch`、浏览器仓库时直接跳这里
- [`万能版直达`](wiki/00-开始这里与落地形态/复制即用提示词包.md#universal-mode)：宿主抓不了网页时直接跳这里
...
第一次上手先看 [`第一次上手指南`](wiki/00-开始这里与落地形态/复制即用提示词包.md)；如果你已经跑通一轮并想进入更稳定的双端固化形态，再看 [`最小稳定闭环指南`](wiki/00-开始这里与落地形态/最小稳定闭环指南.md)；想看详细方法论展开，再看 [`最小闭环`](wiki/02-最小闭环与核心礼法/最小闭环：一次审计版与多次审计版.md)。看图时只记住一句：Web 不直接放行施工，Web 只回判断；真正放行与成立完成事实，都在人类手里。
...
| 模块 | 解决什么问题 |
|------|-------------|
| [00-入口](wiki/00-开始这里与落地形态/) | 第一次怎么跑通、跑通后怎么进入稳定闭环、三件东西各是什么、双端怎么接入 |
...
**00-入口：**
- [第一次上手指南](wiki/00-开始这里与落地形态/复制即用提示词包.md)：最小路由、最小闭环、推荐版、万能版、纠偏词都在这里
- [最小稳定闭环指南](wiki/00-开始这里与落地形态/最小稳定闭环指南.md)：跑通后怎么看双端一起稳定下来；IDE 端 Skill 与 Web 端固定提示词 / 专有应用必须同时就位
...
- [Skill 接入指南](wiki/00-开始这里与落地形态/Skill接入指南.md)：IDE 端什么时候装、如何装、常见坑
- [Web 审计端固定提示词与专有应用指南](wiki/00-开始这里与落地形态/Web审计端固定提示词与专有应用指南.md)：Web 端如何固定系统提示词与创建 Gem / GPT / 专有应用
...
- [First-Time Guide](wiki-en/00-entry/prompt-pack.md): start here for your first runnable loop; use two prompts and prove the protocol can run once
- [Minimal Stable Loop Guide](wiki-en/00-entry/stable-loop-guide.md): go here after the first run; IDE-side Skill and Web-side fixed prompts / app containers must both be in place, and releases must ship both sides together
- [Jump to Recommended Mode](wiki-en/00-entry/prompt-pack.md#repo-link-mode): go straight here when the host can read repo links, use `webfetch`, or open the repository in a browser
- [Jump to Universal Mode](wiki-en/00-entry/prompt-pack.md#universal-mode): go straight here when the host cannot fetch the web
...
graph TD
    ...
    B --> C[IDE executor returns the Atomic Execution Contract and boundaries]
...
Start with the [First-Time Guide](wiki-en/00-entry/prompt-pack.md) for your first run; if you want the stable dual-end setup after that, continue with the [Minimal Stable Loop Guide](wiki-en/00-entry/stable-loop-guide.md); use [Minimal Loop](wiki-en/02-how/minimal-loop.md) when you want the expanded methodology. Read the diagram with one rule in mind: the Web side does not directly release implementation; it only returns judgment, and the human remains the sovereign router and final arbiter.
...
|--------|----------------|
| [00-Entry](wiki-en/00-entry/) | How to get your first run working, how to enter the stable loop after that, what the three pieces are, and how the two ends get set up |
...
- [Minimal Stable Loop Guide](wiki-en/00-entry/stable-loop-guide.md): what counts as a stable loop, why IDE-side Skill plus the Web-side fixed prompt must both be present, and why releases must pair them
...
- [Web Auditor App Guide](wiki-en/00-entry/web-auditor-app-guide.md): how to fix the Web-side prompt into a Gem, GPT, or other app container
