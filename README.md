...
3. 把清单、约束、日志与关键上下文交给 Web 审计位复核
4. 审核通过后，执行位再开始施工
5. 施工结束后，用真实日志、工件、测试结果、Git 记录重新送审
6. 若任一窗口开始腐烂，就中断并续命
...
如果它能把任务拆成“定位保存成功条件 -> 修复提示触发 -> 补一条回归检查”，你再放它开工；做完后不要只听“已经修好”，而要收：当前运行的测试结果、截图或日志、相关提交记录。
